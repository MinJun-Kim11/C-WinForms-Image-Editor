...
// -----------------------------------------------------------------------------
// 파일 역할: ShearFilter 필터의 실제 픽셀 처리 알고리즘을 구현한다.
// -----------------------------------------------------------------------------
...
namespace Photo
{
    ...
    {
        /// <summary>
        /// Shear 왜곡의 실제 픽셀 연산을 담당하는 필터 클래스.
        /// Command 계층에서 호출되며 여기서는 순수하게 계산만 수행한다.
        /// </summary>
        internal class ShearFilter
        {
            unsafe public static void ShearMasked32bpp_BGRA(
                ...
                fixed (byte* src = srcCopy)
                {
                    ...
                    for (int y = workMinY; y <= workMaxY; y++)
                    {
                        ...
                        for (int x = workMinX; x <= workMaxX; x++)
                        {
                            ...
                            // 중심에서 강하고 바깥에서 0
                            double kx = shearX * falloff * aspectX;
                            double ky = shearY * falloff * aspectY;
                            ...
                            // [x]   1/det [ 1  -kx][x']
                            // [y] =       [-ky  1 ][y']
                            double det = 1.0 - kx * ky;
                            if (Math.Abs(det) < 1e-5)
                                det = (det >= 0.0) ? 1e-5 : -1e-5;

                            double srcDx = (dx - kx * dy) / det;
                            double srcDy = (dy - ky * dx) / det;
